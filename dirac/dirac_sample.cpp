/* dirac_sample.cpp — SD card WAV browsing and loading into SDRAM.
 ...
 * this peripheral either) — plain .bss is AXI SRAM on the standard build. */
daisy::FatFSInterface gFsi;
FATFS *gFs = nullptr;
FIL gFil;
constexpr uint32_t kStageBytes = 8192;
...
/* WavReader over an open FatFS file. */
...
int SampleStore::Scan()
{
    ...
    /* Prefer 0:/dirac, fall back to the card root. */
    DIR dir;
    ...
    for(; sd[n] && n < 8; ++n)
        path[n] = sd[n];
    CopyStr(path + n, "dirac", int(sizeof(path)) - n);
    ...
    while(count_ < kMaxFiles)
    {
        ...
        if(!EndsWithWav(fno.fname))
            continue;
        CopyStr(names_[count_], fno.fname, kNameLen);
        count_++;
    }
    f_closedir(&dir);
    status_ = count_ ? "ready" : "no wavs";
    return count_;
}

bool SampleStore::Load(int index, float engineRate, DiracSample &out)
{
    out = DiracSample{};
    truncated_ = false;
    progress_ = 0;
    ...
    WavInfo info;
    const WavResult res = ParseWav(reader, info);
    if(res != WavResult::Ok)
    {
        f_close(&gFil);
        status_ = WavResultName(res);
        return false;
    }
    ...
    while(done < want)
    {
        ...
        const uint32_t bytes = chunk * frameBytes;
        const uint32_t got = reader.Read(gStage, bytes);
        if(got < frameBytes)
        ...
        /* 32-bit only: a 64-bit divide would link in __udivmoddi4. */
        const uint32_t pcStep = (want >= 100u) ? (want / 100u) : 1u;
        progress_ = int(done / pcStep);
        if(haveFrames < chunk)
            break;
    }
    f_close(&gFil);
