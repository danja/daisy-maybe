...
- **resonators** *(awesome)*: Dual delay‑line resonators with 1V/oct tracking, centre-off bipolar feedback on Knob 2/CV 2, wavefolding, filter damping, clock-size scaling, normalised multi-tap output, a second-channel pitch offset, and calibration. Plus five struck bodies (beam, marimba, drumhead, membrane, plate), WAV excitation samples from the SD card, and a Diag page of live meters. MIDI note pitch/velocity is implemented but not yet verified on hardware. Targets QSPI flash — needs the Electrosmith bootloader (`make program-boot`) once. 
